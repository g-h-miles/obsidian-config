...
var DEFAULT_SETTINGS = {
  ...
  folderSuggestionSettings: {
    folderSuggestionMode: FolderSuggestionMode.Always,
    folderSuggestionTrigger: '/',
  },
...
var ObsidianFolderPath = class extends ObsidianPath {
  ...
  static getFolderName(fullPath) {
    ...
    if (folderNameStartsAt === fullPath.length - 1) {
      folderNameStartsAt = fullPath
        .slice(0, folderNameStartsAt)
        .lastIndexOf('/');
  ...
  isAncestorOf(path) {
    ...
    return path.VaultPath.toLowerCase().startsWith(
      this.VaultPath.toLowerCase()
    );
  }
};
...
var FolderSuggest = class extends TextInputSuggest {
  getSuggestions(inputStr) {
    ...
    console.log('AB', abstractFiles);
    const folders = [];
    const lowerCaseInputStr = inputStr.toLowerCase();
    abstractFiles.forEach((folder) => {
      if (folder instanceof import_obsidian2.TFolder) {
        const lowerCaseFolderPath = folder.path.toLowerCase();
        const queryCouldBeForThisFolder = () =>
          lowerCaseFolderPath.contains(lowerCaseInputStr);
        const queryIsAlsoForThisFolder = () =>
          lowerCaseInputStr.endsWith('/') &&
          `${lowerCaseFolderPath}/` === lowerCaseInputStr;
  ...
  renderSuggestion(folder, el) {
    ...
    const pathToShow =
      this.addFinalFolderDelimiterToFolderNameIfNecessary(folderPath);
...
var FileSuggester = class extends TextInputSuggest {
  ...
  getSuggestions(inputStr) {
    ...
    for (const file of files) {
      ...
      const queryCouldBeForThisFile = () =>
        lowerCasePath.contains(lowerCaseInputStr);
...
var SettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  addSuggestionTriggerSetting(containerEl) {
    const trigger = new import_obsidian3.Setting(containerEl)
      .setName('Trigger for link insertion')
      .setDesc('The text string that will trigger link selection.')
      .setTooltip(
        'The string can contain multiple symbols such as @@ Avoid using characters / strings you often use while writing or that might be used by Obsidian or by other plugins to trigger actions. Some examples of strings to avoid: "[", "|", "#"'
      );
    trigger.addText((component) =>
      component.setValue(this.plugin.settings.triggerSymbol).onChange((value) =>
  ...
  addSuggestNonExistingNotesSetting(containerEl) {
    new import_obsidian3.Setting(containerEl)
      ...
      .setDesc(
        'Set this to false if you do not want to get suggestions for existing links to notes that do not exist.'
      )
      .addToggle((component) =>
        component
  ...
  addTemplaterTriggerSetting(containerEl) {
    new import_obsidian3.Setting(containerEl)
      ...
      .setDesc(
        "The text string that will trigger execution of a Tempalter template. Leave empty if you don't need the ability to trigger Templater templates"
      )
      .addText((component) =>
        component
  ...
  addDefaultTemplaterTemplateSetting(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl)
      ...
      .setDesc(
        'This template will be the first in the list of templates to select from when triggering Templater template execution'
      )
      .addSearch((cb) => {
        searchComponent = cb;
        new FileSuggester(
          ...
          this.configStore.getTemplaterTemplatesPath(),
          this.fileSystem
        );
  ...
  addDefaultQuickAddTemplateSetting(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl)
      ...
      .setDesc(
        'This template will be the first in the list of templates to select from when triggering QuickAdd template execution'
      )
      .addSearch((cb) => {
        searchComponent = cb;
        new FileSuggester(
          this.app,
          searchComponent.inputEl,
          this.configStore.getQuickAddTemplatesPath(),
          this.fileSystem
        );
  ...
  addQuickAddTriggerSetting(containerEl) {
    new import_obsidian3.Setting(containerEl)
      ...
      .setDesc(
        "The text string that will trigger execution of a QuickAdd template. Leave empty if you don't need the ability to trigger QuickAdd templates"
      )
      .addText((component) =>
        component
  ...
  addFolderSearchTriggerSetting(containerEl) {
    ...
    if (this.plugin.settings.includeFoldersInSuggestions) {
      const folderSuggestionSettings =
        (_a = this.plugin.settings.folderSuggestionSettings) != null
          ? _a
          : {
              folderSuggestionMode: FolderSuggestionMode.Always,
              folderSuggestionTrigger: '/',
          ...
          dropdown
            ...
            .onChange((value) =>
              __async(this, null, function* () {
                ...
                this.plugin.settings.folderSuggestionSettings =
                  folderSuggestionSettings;
                yield this.plugin.saveSettings();
                this.display();
              })
      ...
      if (
        folderSuggestionSettings.folderSuggestionMode ===
        FolderSuggestionMode.OnTrigger
      ) {
        new import_obsidian3.Setting(containerEl)
          .setName('Trigger for showing only folders')
          .setDesc(
            `The text string that will trigger folder suggestions to be shown. Note that it will only trigger if this is the first symbol you write after writing '${this.plugin.settings.triggerSymbol}'`
          )
          .addText((component) =>
            component
  ...
  warnIfTriggerIsProblematic(value, trigger, component) {
    const triggerStartsWithProblematicCharacter = this.problematicSymbols.some(
      (problem) => value.startsWith(problem)
    );
    const triggerIsEmptyOrOnlyWhitespace = value.trim().length === 0;
    const triggerIsProblematic =
      triggerStartsWithProblematicCharacter || triggerIsEmptyOrOnlyWhitespace;
    if (triggerIsProblematic) {
      trigger.controlEl.addClass('setting-warning');
      component.inputEl.setCustomValidity(
        `Using '${value}' as the trigger for inserting links might not work as intended`
      );
...
var ObsidianInterop = class {
  ...
  generateMarkdownLink(file, sourcePath, subpath, alias) {
    return this.fileSystem.generateMarkdownLink(
      file,
      sourcePath,
      subpath,
      alias
  ...
  getOrCreateFileAndFoldersInPath(creationCommand, currentFile) {
    return __async(this, null, function* () {
      return this.fileSystem.getOrCreateFileAndFoldersInPath(
        creationCommand,
        currentFile
      );
    });
  ...
  createFolderIfNeeded(creationCommand) {
    return __async(this, null, function* () {
      try {
        yield this.app.vault.createFolder(
          creationCommand.PathToNewFolder.VaultPath
        );
      } catch (e) {
        console.debug(
          'NAC: Failed folder creation. Folder probably already exist.'
        );
      }
    });
  }
  tryCreateFile(creationCommand) {
    return __async(this, null, function* () {
      console.debug(
        `NAC: Note does not exist. Will be created. Path: ${creationCommand.PathToNewFile}`
      );
      try {
        return yield this.app.vault.create(
          creationCommand.PathToNewFile,
          creationCommand.NoteContent
        );
      } catch (e) {
        console.debug(
          'NAC: Failed file creation. File probably already exist.'
        );
      }
      return void 0;
    });
  ...
  getHeadersIn(filePath) {
    ...
    const headingCache =
      (_a = this.app.metadataCache.getFileCache(file).headings) != null
        ? _a
        : [];
    return headingCache;
  }
...
var ObsidianFilePath = class extends ObsidianPath {
  get IsRoot() {
    console.log('isroot', this.VaultPath);
    return this.VaultPath === '' || this.VaultPath === '/';
  }
  constructor(path) {
    const fullPath = path.trim();
    ...
    } = ObsidianFilePath.extractPathParts(fullPath);
    ...
    this.VaultPathWithoutExtension = this.NoteIsInRoot
      ? title
      : `${folderPath}/${title}`;
  }
  static extractPathParts(fullPath) {
    ...
    const [folderPath, fileNameWithPossibleExtension] =
      fileNameStartsAt === -1
        ? ['', vaultPath]
        : [
            vaultPath.slice(0, fileNameStartsAt),
            vaultPath.slice(fileNameStartsAt + 1),
          ];
...
var SuggestionRenderer = class {
  static RenderSuggestion(el, params) {
    ...
    if (params.flair) {
      ...
      flair.ariaLabel =
        (_a = params.flair.label) != null ? _a : flair.ariaLabel;
      flair.innerHTML =
        (_b = params.flair.innerHTML) != null ? _b : flair.innerHTML;
    }
  }
};
...
var NoteSuggestion = class extends FileSuggestion {
  ...
  extractSuggestionParts(trigger) {
    ...
    alias =
      (alias == null ? void 0 : alias.length) === 0
        ? void 0
        : alias == null
        ? void 0
        : alias.trim();
...
AliasNoteSuggestion.ALIAS_ICON =
...
var LinkCreationPreparer = class {
  ...
  prepareNoteCreationForTemplateNote(suggestion, currentFile) {
    return __async(this, null, function* () {
      const templateContent = yield this.fileSystem.getFileContentOf(
        suggestion.VaultPath
      );
      return this.prepareNoteCreation(
  ...
  prepareNoteCreation(suggestion, currentFile, getNoteContent) {
    ...
    if (
      suggestion.NoteIsInRoot &&
      suggestion.Title !== '' &&
      !suggestion.Trigger.startsWith('/')
    ) {
      return this.createLinkToNoteInDefaultLocation(
        suggestion,
        currentFile,
        getNoteContent
      );
    }
    return this.createLinkToNoteInSubfolder(
      suggestion,
      noteExists,
      getNoteContent
    );
  ...
  createLinkToNoteInSubfolder(suggestion, noteExists, getNoteContent) {
    ...
    const folderCreationNeeded =
      !suggestion.FolderPath.IsRoot &&
      !this.fileSystem.folderExists(suggestion.FolderPath);
    ...
    const noteCreationCmd = fileCreationNeeded
      ? {
          ...
          PathToNewFile: this.getFileName(suggestion),
        }
      : void 0;
    return {
  ...
  getPathToFileInDefaultFolder(suggestion, currentFile) {
    ...
    switch (defaultNoteLocation) {
      case 'current': {
        const suggestionForCurrentFile = new ExistingNoteSuggestion(
          currentFile.path
        );
      ...
      case 'folder': {
        ...
        return defaultFolder &&
          ...
          this.fileSystem.folderExists(new ObsidianFolderPath(defaultFolder))
...
function extractSuggestionTrigger(lineText, cursorPosition, triggerSymbol) {
  const safeTriggerSymbol = triggerSymbol.replace(
    /[-/\\^$*+?.()|[\]{}]/g,
    '\\$&'
  );
...
var TemplateSuggestion = class extends FileSuggestion {
  constructor(
    ...
    this.rootTemplateFolder =
      rootTemplateFolder != null ? rootTemplateFolder : '';
    this.templateIsInTemplateFolder = this.VaultPath.toLowerCase().includes(
      this.rootTemplateFolder.toLowerCase()
    );
  ...
  static topFolderCheck(queryPath, context, settings) {
    if (settings.relativeTopFolders.length > 0) {
      ...
      const topFolderToUse = settings.relativeTopFolders.find((folder) => {
        return (
          folder.isAncestorOf(filePath) ||
          filePath.FolderPath.VaultPath.toLowerCase().includes(folder.VaultPath)
        );
      });
      if (topFolderToUse) {
        const endOfFolderPath = filePath.FolderPath.VaultPath.lastIndexOf(
          topFolderToUse.VaultPath
        );
        const folderPathToUse = filePath.FolderPath.VaultPath.slice(
          0,
          endOfFolderPath + topFolderToUse.VaultPath.length
        );
...
function getMatcherForExactMatch(lowerCaseQueryPath) {
  return (suggestion) =>
    suggestion.Path.VaultPathWithoutExtension.toLowerCase() ===
    lowerCaseQueryPath.VaultPathWithoutExtension;
}
function getMatcherForPartialMatch(lowerCaseQueryPath) {
  return (suggestion) => {
    const path = suggestion.Path;
    const queryIsAncestor = path.FolderPath.VaultPath.toLowerCase().includes(
      lowerCaseQueryPath.FolderPath.VaultPath
    );
...
var FileQuery = class extends Query {
  ...
  static forNoteSuggestions(context, settings) {
    ...
    console.log('HERE', lowerCaseQueryPath);
    ...
    return new FileQuery(
      query,
      fullMatchFoundChecker,
      partialMatchFoundChecker
    );
  }
  static forTemplateSuggestions(query) {
    const lowerCaseQueryPath = new ObsidianFilePath(query.toLowerCase());
    const fullMatchFoundChecker = getMatcherForExactMatch(lowerCaseQueryPath);
    const partialMatchFoundChecker =
      getMatcherForPartialMatch(lowerCaseQueryPath);
    return new FileQuery(
      query,
      fullMatchFoundChecker,
...
var TemplateSuggestionCollector = class {
  ...
  getSuggestions(templateQuery, noteSuggestion) {
    ...
    const showDefaultFolderFirst =
      templateQuery === '' && defaultTemplate !== '';
    const pathToFilterOut = showDefaultFolderFirst ? defaultTemplate : '';
    for (const suggestion of this.getAllPossibleSuggestions(
      templateFolderPath,
      noteSuggestion,
      pathToFilterOut
    )) {
    ...
    if (showDefaultFolderFirst) {
      const defaultTemplateSuggestion = new TemplateSuggestion(
        defaultTemplate,
        noteSuggestion,
        templateFolderPath,
        this.templateConfig
  ...
  getAllPossibleSuggestions(
    ...
    return templateFolderPath
      ? this.fileSystem
          .getAllFileDescendantsOf(templateFolderPath)
...
var HeaderSuggestionCollector = class {
  ...
  getSuggestions(headerQuery, noteSuggestion) {
    ...
    const headersInNote = this.metadataCollection.getHeadersIn(
      noteSuggestion.Path.VaultPath
    );
    ...
    return headersInNote
      ...
      .map(
        (h) =>
          new HeaderSuggestion(h.heading, h.level, aliasToUse, noteSuggestion)
      );
  }
};
...
var NoteSource = class {
  ...
  createSuggestionFromQuery(query, existingSuggestionForQuery) {
    ...
    if (existingSuggestionForQuery) {
      const suggestionToAdd = queryAsSuggestion.HasAlias
        ? new ExistingNoteSuggestion(
            `${existingSuggestionForQuery.VaultPath}|${queryAsSuggestion.Alias}`
          )
  ...
  getAllPossibleSuggestions(query) {
    ...
    for (const linkSuggestion of linkSuggestions) {
      addIfPathHasNotBeSeen(
        linkSuggestion.path,
        linkSuggestion.file !== null,
        linkSuggestion.alias
      );
    }
  ...
  // getAllPossibleSuggestions(noteSuggestion, pathToFilterOut) {
  //   const templateFolderPath = "00 Resource/People"; // Hardcoded folder path
...
var NoteAndFolderSource = class {
  constructor(interop, settings) {
    ...
    this.sources = [this.noteSource, this.folderSource];
  }
  createSuggestionFromQuery(query, existingSuggestionForQuery) {
    if (
      existingSuggestionForQuery &&
      existingSuggestionForQuery instanceof FolderSuggestion
    ) {
      return this.folderSource.createSuggestionFromQuery(
        query,
        existingSuggestionForQuery
      );
    }
    return this.noteSource.createSuggestionFromQuery(
      query,
      existingSuggestionForQuery
    );
  }
...
var BaseSuggestionCollector = class {
  ...
  getSuggestions(query) {
    ...
    const allPossibleSuggestions =
      this.suggestionSource.getAllPossibleSuggestions(query.query);
    ...
    validSuggestions.unshift(
      this.suggestionSource.createSuggestionFromQuery(
        query,
        existingSuggestionForQuery
      )
    );
...
var FolderQuery = class extends Query {
  constructor(context, settings) {
    ...
    const fullMatchFoundChecker = (suggestion) =>
      suggestion.Path.VaultPath.toLowerCase() === lowerCaseQueryPath.VaultPath;
    const partialMatchFoundChecker = allTrue([
      Query.topFolderCheck(lowerCaseQueryPath, context, settings),
      anyTrue([
        (suggestion) =>
          suggestion.Path.VaultPath.toLowerCase().includes(
            lowerCaseQueryPath.VaultPath
          ),
      ]),
    ]);
...
var _NoteAndFolderQuery = class extends Query {
  constructor(context, settings) {
    ...
    const fullMatchFoundCheckers = [
      allTrue([
        _NoteAndFolderQuery.isNoteSuggestion,
        noteQuery.fullMatchFoundChecker,
      ]),
      allTrue([
        _NoteAndFolderQuery.isFolderSuggestion,
        folderQuery.fullMatchFoundChecker,
      ]),
    ];
    const partialMatchFoundCheckers = [
      allTrue([
        _NoteAndFolderQuery.isNoteSuggestion,
        noteQuery.partialMatchFoundChecker,
      ]),
      allTrue([
        _NoteAndFolderQuery.isFolderSuggestion,
        folderQuery.partialMatchFoundChecker,
      ]),
    ];
    super(
      context.query,
      anyTrue(fullMatchFoundCheckers),
      anyTrue(partialMatchFoundCheckers)
    );
  }
};
var NoteAndFolderQuery = _NoteAndFolderQuery;
NoteAndFolderQuery.isNoteSuggestion = (suggestion) =>
  suggestion instanceof NoteSuggestion;
NoteAndFolderQuery.isFolderSuggestion = (suggestion) =>
  suggestion instanceof FolderSuggestion;
...
var SuggestionCollector = class {
  constructor(interOp, settings) {
    this.settings = settings;
    this.noteSuggestionCollector = new NoteSuggestionCollector(
      interOp,
      settings
    );
    ...
    this.templaterTemplateSuggestionCollector = new TemplateSuggestionCollector(
      interOp,
      interOp,
      settings,
      templaterConfig
    );
    this.quickaddTemplateSuggestionCollector = new TemplateSuggestionCollector(
      interOp,
      interOp,
      settings,
      quickaddConfig
    ...
    this.combinedSuggestionCollector = new NoteAndFolderSuggestionCollector(
      interOp,
      settings
    );
  ...
  getNoteSuggestionFor(query) {
    ...
    console.log('VAULT PATH', tempSuggestion.VaultPath);
    return this.fileSystem.noteExists(tempSuggestion.VaultPath)
      ? tempSuggestion
      : new NewNoteSuggestion(query);
  }
  getSuggestions(context) {
    const q = context.query + (context.query.includes(' ') ? '$' : '');

    if (this.settings.enableRelativePaths && q.startsWith('.')) {
      this.replaceRelativePathInQueryWithFullPath(context);
    }
    let suggestions = [];
    if (
      this.configStore.templaterIsEnabled &&
      // this.settings.templateTriggerSymbol !== "" &&

      q.includes(this.settings.templateTriggerSymbol)
    ) {
      suggestions = this.getTemplaterTemplateSuggestions(q);
    } else if (
      this.configStore.quickAddIsEnabled &&
      this.settings.quickAddTriggerSymbol !== '' &&
      q.includes(this.settings.quickAddTriggerSymbol)
    ) {
      suggestions = this.getQuickAddTemplateSuggestions(q);
    } else if (q.includes('#')) {
      suggestions = this.getHeaderSuggestions(q);
    } else if (this.settings.includeFoldersInSuggestions) {
      suggestions = this.getFoldersOrCombinedSuggestions(q, context);
    } else {
    ...
    return suggestions.length > 0
      ? suggestions
      : [new NotFoundSuggestion(context.query, 'No match found')];
  }
  replaceRelativePathInQueryWithFullPath(context) {
    if (context.query.startsWith('..')) {
      ...
      context.query = context.query.replace(
        '..',
        path.FolderPath.getParentOrThis().VaultPath
      );
  ...
  getNoteSuggestions(context) {
    ...
    return this.noteSuggestionCollector.getSuggestions(
      FileQuery.forNoteSuggestions(context, this.settings)
    );
  }
  getFoldersOrCombinedSuggestions(query, context) {
    const onlyCollectFoldersOnTrigger =
      this.settings.folderSuggestionSettings.folderSuggestionMode ===
      FolderSuggestionMode.OnTrigger;
  ...
  getFolderAndNoteSuggestions(context) {
    return this.combinedSuggestionCollector.getSuggestions(
      new NoteAndFolderQuery(context, this.settings)
    );
  }
  getOnlyFolderOrNoteSuggestions(query, context) {
    const trigger =
      this.settings.folderSuggestionSettings.folderSuggestionTrigger;
  ...
  getFolderSuggestions(context) {
    return this.folderSuggestionCollector.getSuggestions(
      new FolderQuery(context, this.settings)
    );
  }
  ...
  getHeaderSuggestions(query) {
    ...
    if (noteSuggestion instanceof ExistingNoteSuggestion) {
      return this.headerSuggestionCollector.getSuggestions(
        headerQuery,
        noteSuggestion
      );
    } else {
      return [
        new NotFoundSuggestion(
          query,
          'No headers to link to in non-existing notes'
        ),
      ];
    }
  }
  getTemplaterTemplateSuggestions(query) {
    const cleanQry = query + (query.includes(' ') ? '$' : '');
    console.log('*********Q*************', cleanQry);
    const [noteQuery, templateQuery] = cleanQry.split(
      this.settings.templateTriggerSymbol
    );
    const noteSuggestion = this.getNoteSuggestionFor(noteQuery);
    return this.templaterTemplateSuggestionCollector.getSuggestions(
      templateQuery,
      noteSuggestion
    );
  }
  getQuickAddTemplateSuggestions(query) {
    ...
    const noteSuggestion = this.getNoteSuggestionFor(noteQuery);
    return this.quickaddTemplateSuggestionCollector.getSuggestions(
      templateQuery,
      noteSuggestion
    );
  }
...
var LinkSuggestor = class {
  constructor(interop, settings) {
    ...
    this.suggestionsCollector = new SuggestionCollector(
      this.obsidianInterop,
      settings
    );
    this.noteCreationPreparer = new LinkCreationPreparer(
      this.obsidianInterop,
      this.obsidianInterop
    );
    this.settings = settings;
  ...
  getSuggestions(context) {
    // If using @ trigger, force search in People folder
    if (this.settings.triggerSymbol === '@') {
      const peopleFolderPath = '02 Resource/People';
      const files =
        this.obsidianInterop.getAllFileDescendantsOf(peopleFolderPath);

      const searchText = context.query.toLowerCase();
      const suggestions = files
        .filter((file) => {
          const fileName = file.path
            .replace(`${peopleFolderPath}/`, '')
            .toLowerCase();
          return fileName.includes(searchText);
        })
        .map((file) => new ExistingNoteSuggestion(file.path));

      if (suggestions.length > 0) {
        return suggestions;
      }

      // If no matches, allow creating a new person note
      const newNotePath = `${peopleFolderPath}/${context.query}.md`;
      return [new NewNoteSuggestion(newNotePath)];
    }

    // Original suggestion logic for other triggers
    return this.suggestionsCollector.getSuggestions(context);
  }

  onTrigger(cursor, editor, file) {
    const trigger = extractSuggestionTrigger(
    ...
    return {
      ...
      query: trigger.query,
      file,
      triggerSymbol: this.settings.triggerSymbol,
    };
  }
  ...
  selectSuggestion(value, evt, context) {
    return __async(this, null, function* () {
      if (value instanceof TemplateSuggestion) {
        const file =
          yield this.noteCreationPreparer.prepareNoteCreationForTemplateNote(
            value,
            context.file
          );
        if (file) {
          yield this.obsidianInterop.runTemplaterOn(file);
        }
      } else if (value instanceof NoteSuggestion) {
        const file =
          yield this.noteCreationPreparer.prepareNoteCreationForEmptyNote(
            value,
            context.file
          );
        if (file) {
  ...
  get instructions() {
    return [
      {
        command: '↵',
        purpose: 'to create/insert link',
      },
    ];
  }

  updateSuggestionLine(suggestion, context) {
    const editor = app.workspace.activeEditor.editor;
    editor.replaceRange(
      suggestion.textToInsertOnLineUpdate,
      context.start,
      context.end
    );
  }
...
var LinkSuggestorInterop = class extends import_obsidian6.EditorSuggest {
  constructor(app2, settings, configStore, fileSystem) {
    super(app2);
    this.wrapped = new LinkSuggestor(
      new ObsidianInterop(app2, configStore, fileSystem),
      settings
    );
    // Only set instructions if they exist
    if (this.wrapped.instructions && this.wrapped.instructions.length > 0) {
    ...
    this.scope.register([], 'Tab', (event) => {
      ...
      const highlightedSuggestion =
        suggestionCollection.values[idOfHighlightedSuggestion];
  ...
  selectSuggestion(value, evt) {
    ...
    const start = {
      ...
      ch: this.context.start.ch - 1, // Move start back one character to include @
    };

    // Don't try to run templater on existing notes
    if (value instanceof ExistingNoteSuggestion) {
      // Just insert the link and close
      editor.replaceRange(
        `[[${value.Title}]]`, // Add wiki link brackets
        start,
        this.context.end
      );
      this.close(); // Close the suggestion window
      return;
    }

    // For new notes in the People folder, use the default template
    if (
      value instanceof NewNoteSuggestion &&
      value.VaultPath.startsWith('02 Resource/People/')
    ) {
      const defaultTemplate = '04 Templates/Person.md';
      editor.replaceRange(
        `[[${value.Title}]]`, // Add wiki link brackets
        start,
        this.context.end
      ...
      // Create the new note with template
      this.wrapped.obsidianInterop
        .getOrCreateFileAndFoldersInPath(
          {
            NoteCreationCommand: {
              PathToNewFile: value.VaultPath,
              NoteContent: '',
            },
            FullPath: value.VaultPath,
          },
          this.context.file
        )
        .then((file) => {
          if (file) {
            this.wrapped.obsidianInterop
              .getFileContentOf(defaultTemplate)
              .then((templateContent) => {
                if (templateContent) {
                  app.vault.modify(file, templateContent);
                }
              });
          }
        });
...
var ObsidianFileSystem = class {
  ...
  folderExists(folderPath) {
    const foundItem = this.app.vault.getAbstractFileByPath(
  ...
  getPathsToAllLoadedFolders() {
    return app.vault
      .getAllLoadedFiles()
      .filter((f) => f instanceof import_obsidian7.TFolder)
      .map((f) => new ObsidianFolderPath(f.path));
  }
  noteExists(notePath) {
    const foundItem = app.metadataCache.getFirstLinkpathDest(
      notePath,
      app.workspace.getActiveFile().path
    );
...
var NoteAutoCreator = class extends import_obsidian8.Plugin {
  onload() {
    return __async(this, null, function* () {
      ...
      this.addSettingTab(
        new SettingTab(this.app, this, configStore, fileSystem)
      );
      const linkSuggestor = new LinkSuggestorInterop(
        this.app,
        this.settings,
        configStore,
        fileSystem
  ...
  loadSettings() {
    return __async(this, null, function* () {
      ...
      this.settings.relativeTopFolders = this.settings.relativeTopFolders.map(
        (f) => new ObsidianFolderPath(f.VaultPath)
      );
    });
  }
  saveSettings() {
